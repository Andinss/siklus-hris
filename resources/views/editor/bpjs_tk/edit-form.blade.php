...
@section('title', 'Formulit kalkulasi BPJS Ketenagakerjaan')
                    ...
                    <div class="panel panel-info">
                        <div class="panel-wrapper collapse in" aria-expanded="true">
                            <div class="panel-body">
                                <form action="{{ URL::route('editor.bpjs-tk.update', $bpjstk->id) }}" id="formHitungBpjs"
                                    method="POST">
                                    @csrf
                                    <div class="modal-body">
                                        <div class="row mb-3">
                                            <!-- <div class="col-6">
                                              ...
                                              {{ Form::text('basic', old('basic', $bpjstk->basic), ['class' => 'form-control', 'placeholder' => 'Rp', 'id' => 'basic', 'autocomplete' => 'off', 'required']) }}
                                            </div> -->
                                            <div class="col-6">
                                                {{ Form::label('schema_name', 'Nama Skema Perhitungan BPJS TK', ['class' => 'mb-0 mt-3']) }}
                                                {{ Form::text('schema_name', old('schema_name', $bpjstk->schema_name), ['class' => 'form-control', 'placeholder' => 'Contoh: TK Golongan 1/ Staff', 'id' => 'schema_name', 'autocomplete' => 'off', 'required']) }}
                                            </div>
                                        </div>
                                            ...
                                            <tbody id="dtBPJSBody">
                                                @foreach ($details as $key => $row)
                                                    <tr id="dtBPJSRowInitial">
                                                        <td>
                                                            {{ Form::text("bpjs_jaminan[$key]", old('guarantee', $row->guarantee), ['class' => 'form-control', 'placeholder' => 'masukkan nama jaminan', 'id' => 'bpjs_jaminan', 'required', 'autocomplete' => 'off']) }}
                                                        </td>
                                                        <td>
                                                            {{ Form::text("bpjs_ditanggung_perusahaan_persentase[$key]", old('percentage_company', $row->percentage_company), ['class' => 'form-control bpjs_ditanggung_perusahaan_persentase', 'placeholder' => '%', 'id' => "bpjs_ditanggung_perusahaan_persentase_$key", 'required', 'autocomplete' => 'off']) }}
                                                        </td>
                                                        <td>
                                                            {{ Form::text("bpjs_ditanggung_karyawan_persentase[$key]", old('percentage_employee', $row->percentage_employee), ['class' => 'form-control bpjs_ditanggung_karyawan_persentase', 'placeholder' => '%', 'id' => "bpjs_ditanggung_karyawan_persentase_$key", 'required', 'autocomplete' => 'off']) }}
                                                        </td>
                                                        <!-- <td>
                                                  {{ Form::text("bpjs_ditanggung_perusahaan_rp[$key]", old('nominal_company_guarantee', $row->nominal_company_guarantee), ['class' => 'form-control bpjs_ditanggung_perusahaan_rp', 'placeholder' => 'Rp', 'id' => "bpjs_ditanggung_perusahaan_rp_$key", 'readonly' => true]) }}
                                                </td>
                                                <td>
                                                  {{ Form::text("bpjs_ditanggung_karyawan_rp[$key]", old('nominal_employee_guarantee', $row->nominal_employee_guarantee), ['class' => 'form-control bpjs_ditanggung_karyawan_rp', 'placeholder' => 'Rp', 'id' => "bpjs_ditanggung_karyawan_rp_$key", 'readonly' => true]) }}
                                                </td> -->
                                                        <!-- <td>
                                                  <a href="#" role="button" class="btn btn-flat btn-default mb-0">#</a>
                                                </td> -->
                                                    </tr>
                                                @endforeach
                                                <tr>
                                                    ...
                                                    <td>
                                                        {{ Form::text('bpjs_sum_perusahaan_percent', old('bpjs_sum_perusahaan_percent', $bpjstk->total_percentage_company), ['class' => 'form-control', 'placeholder' => '%', 'id' => 'bpjs_sum_perusahaan_percent', 'required', 'autocomplete' => 'off']) }}
                                                    </td>
                                                    <td>
                                                        {{ Form::text('bpjs_sum_karyawan_percent', old('bpjs_sum_karyawan_percent', $bpjstk->total_percentage_employee), ['class' => 'form-control', 'placeholder' => '%', 'id' => 'bpjs_sum_karyawan_percent', 'required', 'autocomplete' => 'off']) }}
                                                    </td>
                                                    <!-- <td>
                                                  {{ Form::text('bpjs_sum_perusahaan', old('bpjs_sum_perusahaan', $bpjstk->total_company_guarantee), ['class' => 'form-control', 'placeholder' => 'Rp', 'id' => 'bpjs_sum_perusahaan', 'readonly' => true]) }}
                                                </td>
                                                <td>
                                                  {{ Form::text('bpjs_sum_karyawan', old('bpjs_sum_karyawan', $bpjstk->total_employee_guarantee), ['class' => 'form-control', 'placeholder' => 'Rp', 'id' => 'bpjs_sum_karyawan', 'readonly' => true]) }}
                                                </td> -->
                                                    <td>
                                                        <div></div>
                                                    </td>
                                                ...
                                                <td>
                                                  {{ Form::text('bpjs_total', old('total_bpjstk', $bpjstk->total_bpjstk), ['class' => 'form-control', 'placeholder' => 'Total Iuran (Rp)', 'id' => 'bpjs_total', 'required']) }}
