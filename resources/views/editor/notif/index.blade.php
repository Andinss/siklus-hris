                            ...
                            @foreach ($notif as $notifs)
                                <div class="media">
                                    <div class="media-left">
                                        <img src="{{ Config::get('constants.path.uploads') }}/user/{{ $notifs->username }}/thumbnail/{{ Auth::user()->filename }}"
                                            class="img-circle" style="width: 50px; height: 50px">
                                    </div>
                                    <div class="media-body">
                                        <strong class="notification-title"><a href="#">{{ $notifs->username }}</a>
                                        </strong>

                                        <b>
                                            @if ($notifs->type == 'Branch Allocation' && $notifs->currency_name != '')
                                                Capital Goods
                                            @else
                                                {{ $notifs->type }}
                                            @endif
                                                ...
                                                <p>
                                                    @php
                                                        foreach ($branch_global as $branch_globals) {
                                                            ...
                                                            echo '&nbsp; ' . $branch_globals->branch_name . ': ';
                                                            echo $notifs->$field_name .
                                                                ' to ' .
                                                                $notifs->$field_nameto .
                                                                ' </br>';
                                                        }
