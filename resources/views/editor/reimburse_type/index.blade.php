...
@section('required', 'errorReimburseType')
@section('content')
    <!-- Page Content -->
    <div id="page-wrapper">
            ...
            <div class="modal-content">
                <form action="#" id="form" class="form-horizontal">
                    ...
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title">Jenis Klaim Form</h4>
                    </div>
                    <div class="modal-body">
                        <div class="form-body">
                            <div class="col-lg-12" class="col-md-12" class="col-sm-12">
                                <label class="control-label col-md-4 pull-left">Jenis Klaim</label>
                                <div class="col-md-8">
                                    <input name="reimburse_type_name" id="reimburse_type_name" class="form-control"
                                        type="text" required>
                ...
                <div class="modal-footer">
                    <div class="col-lg-12 col-md-12 col-sm-12">
                        ...
                        <button type="button" id="btnSaveAdd" onClick="saveadd()" class="btn btn-primary pull-right"> <i
                                class="fa fa-plus-square"></i> Simpan dan Tambah</button>
                    </div>
                </div>
            </div><!-- /.modal-content -->
        </div><!-- /.modal-dialog -->
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            table = $('#dtTable').DataTable({
                ...
                ajax: "{{ URL::route('editor.reimburse-type.data') }}",
                columns: [{
                        data: 'action',
                        name: 'action',
                        orderable: false,
                    ...
                    {
                        data: 'reimburse_type_name',
                        name: 'reimburse_type_name'
                    },
                    {
                        data: 'status',
        ...
        function add() {
            ...
            // $('.modal-title').text('Add Jenis Absensi'); // Set Title to Bootstrap modal title
        }
        ...
        function save() {
            ...
            var url;
            url = "{{ URL::route('editor.reimburse-type.store') }}";
            ...
            $.ajax({
                ...
                data: {
                    ...
                    'reimburse_type_name': $('#reimburse_type_name').val(),
                    'status': $('#status').val()
                },
                success: function(data) {
                    ...
                    if ((data.errors)) {
                        ...
                        if (data.errors.absence_type_name) {
                            $('.@yield('required')').removeClass('hidden');
                            $('.@yield('required')').text(data.errors.absence_type_name);
                        }
                    } else {
        ...
        function edit(id) {
            ...
            $.ajax({
                url: 'reimburse-type/edit/' + id,
                type: "GET",
                dataType: "JSON",
                success: function(data) {

                    $('[name="id_key"]').val(data.id);
                    $('[name="reimburse_type_name"]').val(data.reimburse_type_name);
                    $('[name="status"]').val(data.status);
                    $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
                    $('.modal-title').text('Edit Jenis Absensi'); // Set title to Bootstrap modal title
                },
        ...
        function delete_id(id, reimburse_type_name) {
            var reimburse_type_name = reimburse_type_name.bold();

            $.confirm({
                title: 'Confirm!',
                content: 'Are you sure to delete ' + reimburse_type_name + ' data?',
                type: 'red',
                typeAnimated: true,
                buttons: {
                    cancel: {
