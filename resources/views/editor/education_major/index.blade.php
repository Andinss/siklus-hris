...
@section('title', 'Jurusan Pendidikan')
            ...
            <div class="modal-content">
                <form action="#" id="form" class="form-horizontal">
                    ...
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title">Form Jurusan Pendidikan</h4>
                    </div>
                    <div class="modal-body">
                        <div class="form-body">
                            <div class="col-lg-12" class="col-md-12" class="col-sm-12">
                                <label class="control-label col-md-4 pull-left">Jurusan Pendidikan</label>
                                <div class="col-md-8">
                                    <input name="education_major_name" id="education_major_name" class="form-control"
                                        type="text">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            table = $('#dtTable').DataTable({
                ...
                ajax: "{{ URL::route('editor.education-major.data') }}",
                columns: [{
                        data: 'action',
                        name: 'action',
                        orderable: false,
                    ...
                    {
                        data: 'education_major_name',
                        name: 'education_major_name'
                    },
                    {
                        data: 'status',
                        render: function(data, type, row) {
                            let status = '<span class="label label-success"> Active </span>';
                            if (row.status == 0) {
                                return status;
                            } else {
        ...
        function add() {
            ...
            // $('.modal-title').text('Add Jurusan Pendidikan'); // Set Title to Bootstrap modal title
        }
        ...
        function save() {
            ...
            var url;
            url = "{{ URL::route('editor.education-major.store') }}";
            ...
            $.ajax({
                ...
                data: {
                    ...
                    'education_major_name': $('#education_major_name').val(),
                    'status': $('#status').val()
                },
                success: function(data) {
                    ...
                    if ((data.errors)) {
                        ...
                        if (data.errors.education_major_name) {
                            $('.@yield('required')').removeClass('hidden');
                            $('.@yield('required')').text(data.errors.education_major_name);
                        }
                    } else {
        ...
        function edit(id) {
            ...
            $.ajax({
                url: 'education-major/edit/' + id,
                type: "GET",
                dataType: "JSON",
                success: function(data) {

                    $('[name="id_key"]').val(data.id);
                    $('[name="education_major_name"]').val(data.education_major_name);
                    $('[name="status"]').val(data.status);
                    $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
                    $('.modal-title').text('Edit Jurusan Pendidikan'); // Set title to Bootstrap modal title
                },
        ...
        function delete_id(id, education_major_name) {
            var education_major_name = education_major_name.bold();

            $.confirm({
                title: 'Confirm!',
                content: 'Are you sure to delete ' + education_major_name + ' data?',
                type: 'red',
                typeAnimated: true,
                buttons: {
                    cancel: {
